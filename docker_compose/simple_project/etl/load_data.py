...
from cheking_status import State
...
class PostgresLoader:
    ...
    def get_list_update_filmwork(self, status):
        ...
        stmt = stmt % "WHERE fw.modified > '{}'".format(date) if date else stmt % ''
    ...
    def get_list_update_person(self, status):
        ...
        ORDER BY fw.modified;"""
        date = status.get_state('datetime')
        stmt = stmt % "WHERE p.modified > '{}'".format(date) if date else stmt % ''
        return self.collect_movies(stmt)
    ...
    def get_list_update_genre(self, status):
        ...
        ORDER BY fw.modified;"""
        date = status.get_state('datetime')
        stmt = stmt % "WHERE g.modified > '{}'".format(date) if date else stmt % ''
        return self.collect_movies(stmt)
    ...
    array_agg(DISTINCT fw.id) as films123,
                    ...
                    %s
                    group by person.id, pfw.role"""
        date = status.get_state('datetime')
        stmt = stmt % "WHERE person.modified > '{}'".format(date) if date else stmt % ''
    ...
    def get_list_update_genres(self, status):
        ...
        stmt = """Select genre.id, genre.name,
                    ...
                    group by genre.id"""
        date = status.get_state('datetime')
        stmt = stmt % "WHERE genre.modified > '{}'".format(date) if date else stmt % ''
...
class ElasticSearchSaver:
    ...
    def save_all_data(self, postgres_loader: PostgresLoader, status: State):
        ...
        body_list_persons = 1
        body_list_genres = 1
        last_datetime = datetime.datetime(2, 1, 1)  # Минимальное время
