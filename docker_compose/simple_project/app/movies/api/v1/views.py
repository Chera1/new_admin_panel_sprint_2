    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        for i in range(0, len(queryset)):
            ...
            item['actors'] = [person.full_name for person in
                              persons.filter(personfilmwork__role=TypeRole.actor)]
            item['directors'] = [person.full_name for person in
                                 persons.filter(personfilmwork__role=TypeRole.director)]
            item['writers'] = [person.full_name for person in
                               persons.filter(personfilmwork__role=TypeRole.writer)]
            results.append(item)
